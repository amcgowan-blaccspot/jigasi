 ...
 * Requires one XMPP and one SIP account to be configured in
 * sip-communicator.properties file. JVB conference must be held on the same
 * server as XMPP account. Currently after start the conference held in
 * {@link JvbConference#callContext} MUC is joined. SIP account is used to dial
 * {@link SipGateway} once we join the conference. Work in progress...
 *
 * @author Pawel Domas
 */
public class Main {
    ...
    /**
     * The name of the property that stores the home dir for application log files
     * (not history).
     */
